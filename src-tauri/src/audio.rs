...
pub fn play_audio(file_path: String, id: Option<String>) -> Result<String, String> {
    ...
    thread::spawn(move || {
        match play_audio_internal(&file_path, &audio_id) {
...
pub fn stop_audio(id: String) -> Result<(), String> {
    let players = AUDIO_PLAYERS.lock().map_err(|e| format!("Failed to lock audio players: {}", e))?;
...
fn play_audio_internal(file_path: &str, id: &str) -> Result<(), String> {
    ...
    // Store the sink
    {
        let mut players = AUDIO_PLAYERS.lock()
        ...
        // Get the sink back
        let sink_clone = {
            let mut sink_opt = sink_mutex.lock()
