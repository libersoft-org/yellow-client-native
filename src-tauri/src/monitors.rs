...
unsafe extern "system" fn enum_monitor_proc(
    ...
    if let Some(monitors) = vec_ptr.as_mut() {
        let mut info = MONITORINFOEXA { cbSize: std::mem::size_of::<MONITORINFOEXA>() as u32, ..Default::default() };
        GetMonitorInfoW(hmon, &mut info);
        monitors.push(MonitorInfo { name: info.szDevice.to_string_lossy().into_owned(), area: Area { left: info.rcMonitor.left as u32, top: info.rcMonitor.top as u32, right: info.rcMonitor.right as u32, bottom: info.rcMonitor.bottom as u32 }, work_area: Area { left: info.rcWork.left as u32, top: info.rcWork.top as u32, right: info.rcWork.right as u32, bottom: info.rcWork.bottom as u32 } });
    }
    true.into()
}
